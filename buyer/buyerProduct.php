<?php
$min = 1;
$max = 99999; // Changed to consistent value
...
if(isset($_GET['page']))
    ...
    $num_per_page = 6; // Set to 6 items per page
    $start_from = ($page-1)*$num_per_page; // Calculate start position
...
$sql="select * from product where shelf='on' and quantity>0 and CAST(price AS DECIMAL) BETWEEN '$min' AND '$max' and title like '%$keyword%' ORDER BY CAST(price AS DECIMAL) limit $start_from,$num_per_page";
$result=$conn->query($sql);
}else if(! empty($_POST['category'])){
  $select_category = $_POST['category'];
  $sql="select * from product where shelf='on' and category ='$select_category' and quantity>0 and CAST(price AS DECIMAL) BETWEEN '$min' AND '$max' and title like '%$keyword%' ORDER BY CAST(price AS DECIMAL) limit $start_from,$num_per_page";
  $result=$conn->query($sql);
}
...
<form id="Formid" method="post" action="">
  ...
  <section class="page-section">
    ...
    text-overflow: ellipsis;height:37px;"><?php echo $description;?></p>
                  <p class="tags">RS <?php echo $price;?></p>
        ...
        $select_category="";
if (empty($_POST['category'])) {
  $pr_query = "select * from product where shelf='on' and quantity>0 and CAST(price AS DECIMAL) BETWEEN '$min' AND '$max' and title like '%$keyword%' ORDER BY CAST(price AS DECIMAL)";
  $pr_result=$conn->query($pr_query);
  $total_record = mysqli_num_rows($pr_result);
}else if(! empty($_POST['category'])){
  $select_category = $_POST['category'];
  $pr_query="select * from product where shelf='on' and quantity>0 and category ='$select_category' and CAST(price AS DECIMAL) BETWEEN '$min' AND '$max' and title like '%$keyword%' ORDER BY CAST(price AS DECIMAL)";
  $pr_result=$conn->query($pr_query);
  $total_record = mysqli_num_rows($pr_result);
}
                ...
                for($i=1; $i<=$total_pages; $i++) {
                    ...
                    echo "<a href='buyerProduct.php?page=".$i."' class='btn btn-primary ".$active."' style='margin: 0 5px;'>$i</a>";
                }
